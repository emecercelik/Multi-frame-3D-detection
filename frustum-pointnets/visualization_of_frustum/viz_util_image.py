...
if __name__=='__main__':
    ...
    parser.add_argument('--root_dir_tracking',dest = 'root_dir_tracking', help='Root directory to KITTI Tracking dataset. root_dir should contain:data_tracking_image_2/<split>/image_02/, data_tracking_label_2/<split>/label_02/,data_tracking_velodyne/<split>/velodyne/,data_tracking_calib/<split>/calib/')
    parser.add_argument('--drives', metavar='N', type=int, nargs='+',help='Indices of KITTI tracking dataset drives, from which the images will be generated.')
    parser.add_argument('--save_path', help='Path to save images.')
    parser.add_argument('--pred_path', help='Path to the predictions. This should contain folders as following: pred_path/0000/data/000000.txt,000001.txt,...;0011/data/000000.txt,000001.txt,...')
    parser.add_argument('--gt_path', help='Path to the KITTI tracking ground-truth labels arranged in KITTI object detection format. Inside this folder, there should be folders with drive id such that gt_path/0000/000000.txt,000001.txt,...;0001/000000.txt,000001.txt,...')
    parser.add_argument('--gray_scale', action='store_true', help='To save image as grayscale, otherwise color by default')
    
    
    args = parser.parse_args()
    
    for drive in args.drives:
        img_path = os.path.join(args.root_dir_tracking,\
                                'data_tracking_image_2/training/image_02/{:04d}'.format(drive))
        pred_root_path = os.path.join(args.pred_path,\
                                     '{:04d}/data/'.format(drive))
        img_names = [n for n in os.listdir(pred_root_path) if '.txt' in n] 
        img_names.sort()
        for img_name in img_names:
            img_id = int(os.path.basename(img_name).split('.')[0])
            print('Drive: {}, Img: {}'.format(drive,img_id))
            img_dir = os.path.join(img_path,'{:06d}.png'.format(img_id))
            # Get image
            if args.gray_scale:
            ...
            ## Get calibration
            calib_name = os.path.join(args.root_dir_tracking,\
                                'data_tracking_calib/training/calib/{:04d}.txt'.format(drive))
            calib_mat = get_calib_dict(calib_name)
            
            # Get predictions and object corners
            pred_name = os.path.join(args.pred_path,\
                                     '{:04d}/data/{:06d}.txt'.format(drive,img_id))
            
            pred_objects = get_objects(pred_name)
            pred_corners = []
            ...
            if args.gt_path is not None:
                # Get gt-labels and object corners
                gt_label_name = os.path.join(args.gt_path,"{:04d}/{:06d}.txt".format(drive,img_id))
                ...
                # Draw ground-truth objects on the image
                draw_3d_boxes_on_image(gt_corners,draw,color='green')
            
            # Save image
            save_folder = os.path.join(args.save_path,'{:04d}'.format(drive))
            if not os.path.exists(save_folder): os.makedirs(save_folder)
            save_name = os.path.join(save_folder,'{:06d}.png'.format(img_id))
