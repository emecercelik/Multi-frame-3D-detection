...
def get_loss(mask_label, center_label, heading_class_label, heading_residual_label, size_class_label,
    ...
    #ml = tf.reshape(mask_label,(-1,))
    #ep = tf.reshape(end_points['mask_logits'],(-1,2))
    
    #mask_loss = tf.reduce_mean(focal_loss_softmax(ml,ep))
    ...
    if end_points['lstm_layer']['loss'] is not None:
