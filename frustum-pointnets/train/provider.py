...
class FrustumDataset(object):
    ...
    def __getitem__(self, index):
        ...
        # Resample
        # If statements are for fixing point sampling during validation to have consistent results
        '''
        l_point = len(point_set)
        if l_point<=1000:
            FrustumDataset.point_size_1000+=1
        elif l_point<=2000:
            FrustumDataset.point_size_2000+=1
        elif l_point<=3000:
            FrustumDataset.point_size_3000+=1
        elif l_point<=4000:
            FrustumDataset.point_size_4000+=1
        elif l_point<=5000:
            FrustumDataset.point_size_5000+=1
        elif l_point<=6000:
            FrustumDataset.point_size_6000+=1
        else:
            FrustumDataset.point_size_more+=1
        '''
        #print("Points: {:04d}<=1000, {:04d}<=2000, {:04d}<=3000, {:04d}<=4000, {:04d}<=5000, {:04d}<=6000, {:04d}>6000".format(
        #    FrustumDataset.point_size_1000,FrustumDataset.point_size_2000,FrustumDataset.point_size_3000,
        #    FrustumDataset.point_size_4000,FrustumDataset.point_size_5000,FrustumDataset.point_size_6000,
        #    FrustumDataset.point_size_more))
        
        if not self.random_flip and not self.random_shift:
            #np.random.seed(101) # might be used to replicate results
