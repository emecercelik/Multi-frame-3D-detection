...
'''
Based on https://github.com/xinshuoweng/AB3DMOT
Modified by Emeç Erçelik
'''
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha,pred_path='./results', gt_path="./evaluation", max_truncation = 0, min_height = 25, max_occlusion = 2, mail=None, cls="car", eval_3diou=True, eval_2diou=False):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        filename_test_mapping = os.path.join(pred_path, 'evaluate_tracking.seqmap.val')
        self.n_frames         = []
        self.sequence_name    = []
        ...
        # data and parameter
        self.gt_path           = os.path.join(gt_path)
        self.t_sha             = t_sha
        self.t_path            = os.path.join(pred_path)
...
class stat:
    ...
    def plot(self):
        save_dir = os.path.join(self.t_sha,'result_plots')
        try:
...
def evaluate(result_sha,mail,eval_3diou,eval_2diou,args):
    """
    ...
    # for c in ("car"):
        e = trackingEvaluation(t_sha=result_sha, pred_path=args.path_to_predictions,
                               gt_path=args.path_to_gts,mail=mail,cls=c,
        ...
        stat_meter = stat(t_sha=e.t_path, cls=c, suffix=suffix, dump=dump)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path_to_predictions',help='Path that contains track results in KITTI format. path/0011.txt etc.')
    parser.add_argument('--path_to_gts',help='Path that contains ground-truth track information in KITTI format. path/0011.txt etc.')
    parser.add_argument('--two_d', action='store_true', help='Flag to evaluate 2D tracking. If 3D is also set, this will be overwritten as False.')
    parser.add_argument('--three_d', action='store_true', help='Flag to evaluate 3D tracking.')

    args = parser.parse_args()
    
    if args.three_d and args.two_d:
        args.two_d == False
    elif not args.three_d and not args.two_d:
        args.three_d = True
    eval_2diou = args.two_d
    eval_3diou = args.three_d
    ...
    '''
    # evaluate results
    success = evaluate(result_sha,mail,eval_3diou,eval_2diou,args)
